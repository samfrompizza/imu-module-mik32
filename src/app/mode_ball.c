...
#define BALL_MIN_U (BALL_RADIUS_PX)
#define BALL_MAX_U (DISPLAY_WIDTH - 1 - BALL_RADIUS_PX)
#define BALL_MIN_V (BALL_RADIUS_PX)
...
static void integrate_axis(float *pos, float *vel,
                           float accel, float min, float max)
{
                        ...
                        const imu_frame_t *frame)
{
    ...
    ssd1306_FillCircle(display,
                       (uint8_t)s_ball.pos_u, (uint8_t)s_ball.pos_v,
